...
import Rel8.Query.Evaluate ( eval )
...
catListTable (ListTable as) = eval $ fromColumns $ runIdentity $
...
catNonEmptyTable (NonEmptyTable as) = eval $ fromColumns $ runIdentity $
  hunvectorize (\SSpec {info} -> pure . E . sunnest info . unE) as
...
catList = eval . sunnest typeInformation
...
catNonEmpty = eval . sunnest typeInformation
